...
void client() {
  ...
  if (auto e = connect(fd, (struct sockaddr *)&addr, sizeof(addr)); e < 0) {
    ...
    auto ws1 = write(fd, buf, sizeof(buf));
    if (ws1 < 0) {
    ...
    auto ws = write(fd, buf, sizeof(buf));
    if (ws < 0) {
      printf("write error %s \n", strerror(errno));
      close(fd);
      return;
    ...
    auto ws2 = write(fd, buf, sizeof(buf));
...
void server() {
  ...
  while (true) {
    ...
    auto connectfd = accept(fd, (sockaddr *)&addr, &si);
    ...
    char buf[1024] = {0};
    if (auto e = read(connectfd, buf, sizeof(buf)); e < 0) {
...
int main(int argc, char const *argv[]) {
  ...
  if (auto e = sigprocmask(SIG_SETMASK, &sig, nullptr); e < 0) {
